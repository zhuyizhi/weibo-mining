...
public class Model {	
	...
	public boolean saveModelOthers(String filename){
		try{
			...
			double t= Math.log((double)M)-K*V*logGamma(beta)+K*logGamma(V*beta);
			double temp2=t-Math.log((double)M);
			//System.out.println("  initial t:"+t);
			int nt[][]=new int[V][K];
			int b[]=new int[K];
			double t1=0;
			// to compute the P(w|z,T) T are the number of topics.
			for (int i = 0; i < data.M; i++){
				HashMap temp=new HashMap<Type,Integer>();
				int a[]=new int[K];
				for (int j = 0; j < data.docs[i].length; ++j){
					//writer.write(data.docs[i].words[j] + ":" + z[i].get(j) + " ");
					Type type=new Type(data.docs[i].words[j],z[i].get(j));
					if(!temp.containsKey(type))
						temp.put(type, 1);
					else{ 
						temp.put(type, (Integer)temp.get(type)+1);
						}
					++nt[data.docs[i].words[j]][z[i].get(j)];
					++a[z[i].get(j)];
					++b[z[i].get(j)];
				}
				double t2=1;
				for(int k=0;k<K;k++)
				{
					...
					{
					double	t3=1;
					Set<Type> set=temp.keySet();
					for(Type tObject:set){
						if(tObject.y==k)
						{
						//double it=(Integer)temp.get(tObject)+beta;
						//System.out.println(gamma(it));						
						t3*=gamma((Integer)temp.get(tObject)+beta);
						}
					}
					//System.out.println("Initial t3:"+t3);
					t2+=logGamma(a[k]+V*beta)-Math.log(t3);
					//System.out.println("Innitial t2:"+t2);
					}
				}
				t1+=t2;
				//System.out.println("Iintial t1:"+t1);
			}
			t-=t1;
			writer.write("The log(P(z|T))"+t+"\n");
			
			for(int i=0;i<K;i++)
			{
				temp2-=logGamma(b[i]+V*beta);
				for(int j=0;j<V;j++)
					temp2+=logGamma(nt[j][i]+beta);
			}
			writer.write("The log(P(w|z,T)) "+ temp2);
			writer.close();
		}
		catch(Exception e){
