...
{   //主函数
	public static void main(String[] args)
	{
		...
		{
			//字符串分词          
//			String sInput = "随后温总理就板板六十四的离开了舟曲县城，预计温总理今天下午就回到北京。以上就是今天上午的最新动态";
			String sInput = "NBA停摆结束，圣诞节将会开赛，易建联会回去么？";
...
//			testICTCLAS_FileProcess();
	...
	public static void testICTCLAS_ParagraphProcess(String sInput)
	{
		try
		{
			...
			PrintWriter pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream("splitTest.txt"), "UTF-8"));

			//导入用户词典前分词
			byte nativeBytes[] = testICTCLAS50.ICTCLAS_ParagraphProcess
			(sInput.getBytes("UTF-8"), ICTCLAS50.eCodeType.CODE_TYPE_UTF8.ordinal(), 1);//分词处理
			System.out.println(nativeBytes.length);
			String nativeStr = new String(nativeBytes, 0, nativeBytes.length, "UTF-8");
			System.out.println("未导入用户词典的分词结果： " + nativeStr);//打印结果
			pw.println(nativeStr);
			pw.close();
			//导入用户字典
			int nCount = 0;
			String usrdir = "./sentidics/HowNet_sentiDic_small.txt"; //用户字典路径
